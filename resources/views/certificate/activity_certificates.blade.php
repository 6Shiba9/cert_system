        ...
        <table class="min-w-full border-collapse">
            <thead>
                <tr class="bg-gray-200 text-gray-700 uppercase text-sm font-semibold text-center">
                    ...
                    <th class="px-6 py-3 border-b">ชื่อ</th>
                    <th class="px-6 py-3 border-b">อีเมล</th>
                    <th class="px-6 py-3 border-b">จำนวนดาวน์โหลด</th>
                    <th class="px-6 py-3 border-b">ลิงก์ใบประกาศ</th>
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-100 transition">
                    ...
                    <td class="px-6 py-4 text-sm text-gray-700">{{ $participant->email ?? '-' }}</td>
                    ...
                    <td class="px-6 py-4">
                        <div class="flex gap-2 justify-center">
                            <a href="{{ route('certificate.pdf', $participant->certificate_token) }}" 
                               target="_blank"
                               class="bg-blue-600 text-white px-3 py-1.5 rounded-lg text-xs hover:bg-blue-700 transition flex items-center gap-1">
                            ...
                            <a href="{{ route('certificate.pdf.download', $participant->certificate_token) }}" 
                               class="bg-green-600 text-white px-3 py-1.5 rounded-lg text-xs hover:bg-green-700 transition flex items-center gap-1">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <button onclick="copyLink('{{ route('certificate.pdf', $participant->certificate_token) }}')"
                                    class="bg-gray-600 text-white px-3 py-1.5 rounded-lg text-xs hover:bg-gray-700 transition flex items-center gap-1">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div id="add-edit-modal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden flex items-center justify-center z-50">
    <div class="bg-white w-full max-w-md rounded-2xl shadow-2xl p-6 relative">
        ...
        <form id="participant-form" action="{{ route('participant.add', $activity->activity_id) }}" method="POST" class="space-y-4">
            ...
            <div>
                <label for="participant_name" class="block text-sm font-semibold text-gray-700 mb-1">ชื่อ-นามสกุล <span class="text-red-500">*</span></label>
            ...
            <div>
                <label for="participant_email" class="block text-sm font-semibold text-gray-700 mb-1">อีเมล (ถ้ามี)</label>
    ...
    <div class="bg-white w-full max-w-md rounded-2xl shadow-2xl p-6 relative">
        ...
        <form action="{{ route('participants.upload', $activity->activity_id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <p class="text-xs text-blue-800 font-semibold mb-2">📋 รูปแบบไฟล์ Excel:</p>
                <ul class="text-xs text-blue-700 space-y-1">
                    <li>• คอลัมน์ A: ชื่อ-นามสกุล (บังคับ)</li>
                    <li>• คอลัมน์ B: อีเมล (ถ้ามี)</li>
                    <li>• แถวแรกเป็นหัวตาราง</li>
                </ul>
            </div>
