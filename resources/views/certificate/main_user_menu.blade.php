        ...
        <div class="container mx-auto px-6 py-4">
            <div class="flex justify-between items-center">
                ...
                <a href="{{ route('certificate.form') }}"  
                   class="bg-white bg-opacity-10 backdrop-blur-sm hover:bg-white hover:text-blue-600 text-white font-semibold px-4 py-2 rounded-xl border border-white border-opacity-20 transition-all duration-300 shadow-lg">
                    🔍 ค้นหาด้วยรหัส
                </a>
            </div>
        </div>
    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="bg-white rounded-2xl shadow-xl p-8 mb-8">
            <div class="flex items-center gap-4 mb-6">
                ...
                <div>
                    <h1 class="text-3xl font-bold text-gray-800">ค้นหาใบประกาศ/ใบรับรอง</h1>
                    <p class="text-gray-600 mt-1">เลือกกิจกรรมเพื่อดาวน์โหลดใบประกาศของคุณ</p>
                </div>
            </div>
            ...
            <form action="{{ route('user.dashboard') }}" method="GET" class="flex gap-3">
                <input type="text" 
                       ...
                       placeholder="ค้นหาชื่อกิจกรรมหรือหน่วยงาน..." 
                       class="flex-1 px-5 py-4 border-2 border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition">
                <button type="submit" 
                        class="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-semibold px-8 py-4 rounded-xl transition duration-200 shadow-lg flex items-center gap-2">
        ...
        <p class="text-gray-600 mb-6">
            ผลการค้นหา "<span class="font-semibold">{{ request('search') }}</span>" 
            พบ <span class="font-semibold">{{ $activities->count() }} กิจกรรม</span>
        </p>
        @else
        <p class="text-gray-600 mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-300 border border-gray-200 transform hover:-translate-y-2">
                <!-- Certificate Image -->
            ...
            <div class="col-span-full text-center py-16">
                ...
                <p class="text-gray-500">
                    @if(request('search'))
                        ลองค้นหาด้วยคำค้นอื่นหรือติดต่อผู้จัดกิจกรรม
                    @else
                        ยังไม่มีกิจกรรมที่เปิดให้ดาวน์โหลดใบประกาศในขณะนี้
                    @endif
                </p>
            </div>
            @endforelse
        ...
        <!-- Pagination (ถ้ามี) -->
        @if($activities->count() > 0 && method_exists($activities, 'links'))
        <div class="mt-12">
            {{ $activities->links() }}
        </div>
        @endif
